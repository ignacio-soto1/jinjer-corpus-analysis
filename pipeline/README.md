...
A single Python script that runs the four-hypothesis discourse analysis end-to-end:

1. Loads `corpus/metadata.csv` (61 tracks).
2. For each English track: reads the corresponding lyric file from `corpus/local/{filename}`, lemmatises with spaCy, computes per-track metrics for H1 (territory), H2 (displacement), H3 (conflict frame markers), and H4 (gender positional configuration).
3. Aggregates per-track metrics into per-album metrics.
4. Runs inferential tests (Spearman ρ for H1 and H3; Welch t-test for H2; descriptive summary for H4).
5. Writes seven CSVs to `outputs/`.

## Embedded dictionaries

The thematic dictionaries are embedded as Python sets in this script (lines 71–168). See `dictionaries/README.md` for the rationale and the mapping of each set to the hypothesis it supports.
...
— `outputs/hypothesis_tests.csv` (H1, H2, H3 inferential tests)
